...
export default function ProfilePage() {
    ...
    const InfoItem = ({ icon: Icon, label, value }: { icon: any, label: string, value: string | null | undefined }) => (
        <div className="flex items-center gap-4 p-4 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 transition-colors">
            ...
            <div>
                ...
                <p className="text-gray-900 dark:text-gray-100 font-semibold">{value || 'Not set'}</p>
            </div>
        </div>
    )

    const BooleanItem = ({ label, value }: { label: string, value: boolean | null }) => (
        <div className="flex items-center justify-between p-4 bg-white dark:bg-slate-900 rounded-xl shadow-sm border border-gray-100 dark:border-slate-800 transition-colors">
            <span className="text-gray-700 dark:text-gray-300 font-medium">{label}</span>
            <div className={`px-3 py-1 rounded-full text-xs font-bold ${value ? 'bg-green-100 dark:bg-green-900/30 text-green-700 dark:text-green-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-500 dark:text-gray-400'}`}>
                {value ? 'Yes' : 'No'}
            </div>
        </div>
    )
    ...
    return (
        <div className="min-h-screen bg-gray-50 dark:bg-slate-950 text-gray-900 dark:text-gray-100 pb-24 font-sans flex flex-col items-center transition-colors duration-300">
            ...
            <header className="w-full pt-8 pb-4 px-6 flex items-center relative bg-gray-50 dark:bg-slate-950 transition-colors">
                ...
                <h1 className="w-full text-center text-xl font-bold text-gray-900 dark:text-white">Profile Details</h1>
            </header>
            ...
            <main className="w-full max-w-md px-6 space-y-6 mt-4">
                ...
                <section className="space-y-4">
                    <h2 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider ml-1">Personal Information</h2>
                ...
                {/* Emergency Contact */}
                <section className="space-y-4">
                    <h2 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider ml-1">Emergency Contact</h2>
                ...
                <section className="space-y-3">
                    <h2 className="text-sm font-bold text-gray-500 dark:text-gray-400 uppercase tracking-wider ml-1">Household Details</h2>
